...
void PartMatrix::render(Particle_Abstract particles[], byte numParticles)
{
    ...
    for(int i = 0; i<numParticles; i++) {
        ...
        if (col < PS_PIXELS_X && row < PS_PIXELS_Y) {
            tempVal = ((unsigned long)(PS_P_RADIUS-dx)*(PS_P_RADIUS-dy)*particles[i].ttl)/PS_P_SURFACE;//>>10; //divide by PS_P_SURFACE == 1024
            addColor(col, row, &baseRGB, tempVal);
        }
        ...
        col--;
        if (row < PS_PIXELS_Y) {
            tempVal = ((unsigned long)dx*(PS_P_RADIUS-dy)*particles[i].ttl)/PS_P_SURFACE;//>>10; //divide by PS_P_SURFACE == 1024
            addColor(col, row, &baseRGB, tempVal);
        }
    }
}
...
void PartMatrix::addColor(byte col, byte row, ColorRGB *colorRGB, unsigned long value)
{
    ...
    if (colorRGB->r > 0) {
        ...
        matrix[col][row].r = std::min(tempVal, (unsigned long)255);
        if (isOverflow && tempVal > 255) {
            res = tempVal-255;
            res = res>>2;
            matrix[col][row].g = std::min((matrix[col][row].g+res), 255);
            matrix[col][row].b = std::min((matrix[col][row].b+res), 255);
        }
    }
    ...
    if (colorRGB->g > 0) {
        ...
        matrix[col][row].g = std::min(tempVal, (unsigned long)255);
        if (isOverflow && tempVal > 255) {
            res = tempVal-255;
            res = res>>2;
            matrix[col][row].r = std::min((matrix[col][row].r+res), 255);
            matrix[col][row].b = std::min((matrix[col][row].b+res), 255);
        }
    }
    ...
    if (colorRGB->b > 0) {
        ...
        matrix[col][row].b = std::min(tempVal, (unsigned long)255);
        if (isOverflow && tempVal > 255) {
            res = tempVal-255;
            res = res>>2;
            matrix[col][row].r = std::min((matrix[col][row].r+res), 255);
